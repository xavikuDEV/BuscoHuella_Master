...
const MapEngine = dynamic(() => import("./MapInner"), {
  ssr: false,
  loading: () => (
...
interface LiveMapProps {
  ...
  incidents?: any[];
  sector?: string;
  /** jurisdictionId para activar el Grid Táctico Cian */
  gridJurisdictionId?: string | null;
  /** Callback al hacer click derecho / long-press en el mapa */
  ...
  return (
    <div className="h-full w-full relative rounded-[3.5rem] overflow-hidden border border-slate-800 shadow-2xl bg-slate-950 group">
      ...
      <div className="absolute top-8 left-8 z-1000 pointer-events-none space-y-3">
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 p-5 rounded-3xl shadow-2xl">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-black text-white uppercase tracking-tighter">
              {gridJurisdictionId ? "Barcelona_Grid_Radar" : "Sabadell_Grid_Radar"}
            </span>
          </div>
        ...
        {/* Hint Click-to-Alert */}
        {onMapClick && (
          <div className="bg-slate-900/70 backdrop-blur-md border border-cyan-500/20 px-3 py-2 rounded-xl">
            <p className="text-[8px] font-mono text-cyan-400/70 uppercase tracking-widest">
              ▶ Click derecho → Nueva Alerta
            </p>
          </div>
        )}
      </div>
      ...
      <MapEngine
        pets={pets}
        incidents={incidents}
        gridJurisdictionId={gridJurisdictionId}
